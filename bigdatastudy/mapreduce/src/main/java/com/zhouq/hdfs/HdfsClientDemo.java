...
public class HdfsClientDemo {
	...
	public void init() throws Exception{
		...
		//设置客户端访问方式使用主机名 ， 这句代码很很重。如果是链接云服务器上面的hadoop集群必须设置。
...
//		conf.set("fs.defaultFS", "com.zhouq.hdfs://mini1:9000");
		...
		//可以直接传入 uri和用户身份
		fs = FileSystem.get(new URI("com.zhouq.hdfs://mini1:9000"),conf,"hadoop"); //最后一个参数为用户名
...
//		Thread.sleep(2000);
		fs.copyFromLocalFile(new Path("F:/register.rar"), new Path("/register5.rar"));
		fs.close();
	}
	...
	public void testDownload() throws Exception {
		...
		fs.copyToLocalFile(new Path("/register4.rar"), new Path("d:/"));
		fs.close();
	}
	...
	public void makdirTest() throws Exception {
		boolean mkdirs = fs.mkdirs(new Path("/aaa/bbb1"));
