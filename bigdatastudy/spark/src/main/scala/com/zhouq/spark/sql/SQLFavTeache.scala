...
object SQLFavTeacher {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //row_number 不管相同的数据 如: 1,2,3,4,5,6,7
    ...
    //rank 函数相同的数据排同一个名次,但是排序不连续 如: 1,2,3,3,5,6,7
    //    val temp2 = spark.sql(s"SELECT *, rank() over(order by counts desc) g_rk FROM (SELECT subject, teacher, counts, row_number() over(partition by subject order by counts desc) sub_rk FROM v_temp_sub_teacher_counts) temp2 WHERE sub_rk <= $topN")

    //dense_rank 函数 相同的数据排同一个名次,并且顺序连续 如: 1,2,3,3,4,5,6
